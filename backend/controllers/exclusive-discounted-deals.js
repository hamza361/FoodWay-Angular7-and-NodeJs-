...
exports.getProducts = (req,res,next)=>{
    Exclusive_Discounted_Products.find().then((products)=>{
...
exports.addProduct  = (req,res,next)=>{
    ...
    const product = new Exclusive_Discounted_Products({name:name,imagePath:imagePath,price:price,description:description,category:category});
...
exports.deleteProduct = (req,res,next)=>{
    ...
    Exclusive_Discounted_Products.findOne({_id:_id}).then((product)=>{
        if(!product){
            const error = new Error('Fail to find Product');
            throw error;
        }
        product.imagePath = product.imagePath.replace('http://localhost:3000/','backend/')
        fileHelper.deleteFile(product.imagePath,res);
        return Exclusive_Discounted_Products.deleteOne({_id:_id});
   }).then((result)=>{
        if(result.n!=1){
...
exports.getProduct = (req,res,next)=>{
    ...
    Exclusive_Discounted_Products.findById({_id:_id}).then((product)=>{
...
exports.editProduct = (req,res,next)=>{
    ...
    Exclusive_Discounted_Products.updateOne({_id:_id},product).then((result)=>{
        if(!result){
